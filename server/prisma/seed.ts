...
const industries = ["E-commerce", "SaaS", "EdTech", "FinTech", "HealthTech", "FoodTech"];
...
async function main() {
  ...
  // Clean existing data
  await prisma.campaignOpportunity.deleteMany();
  ...
  // Weight towards more failures to populate recovery opportunities
  ...
  // Analyze ~70% of failed transactions
  ...
  for (const cd of campaignData) {
    const oppSlice = opportunities.splice(0, Math.min(randBetween(8, 25), opportunities.length));
